...
function moveSlider(direction) {
    ...
    const maxMoveRight = -(totalCards - maxVisibleCards) * standardCardWidth; // Adjusted for RTL

    if (direction === 'right') {
        if (currentPosition > maxMoveRight) {
            currentPosition -= standardCardWidth; // Move right in RTL
        } else {
            currentPosition = maxMoveRight; // Bounce back to the start
        }
    } else if (direction === 'left') {
        if (currentPosition < 0) {
            currentPosition += standardCardWidth; // Move left in RTL
        } else {
            currentPosition = maxMoveRight; // Bounce back to the end
        }
    }

    sliderWrapper.style.transform = `translateX(${currentPosition}px)`; // Move the slider
